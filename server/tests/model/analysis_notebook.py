...
bool_cols = [
    "match_found",
    "correct_recipient_ids",
    "correct_best_image_id",
    "is_perfect_run",
]
...
is_llama_native = df["model_name"].astype(str).str.lower().str.contains("llama") & (
    df["tesseract_time"].isnull() | (df["tesseract_time"] == 0)
)
...
# %%
agg_df = (
    df.groupby("model_name", observed=False)
    .agg(
...
time_df = (
    df.groupby("model_name", observed=False)[
        ["tesseract_time", "llama_time", "total_time"]
    ]
    .mean()
    .reset_index()
)
...
time_df["bar2_llm"] = time_df.apply(
    lambda r: r["total_time"] if pd.isnull(r["llama_time"]) else r["llama_time"], axis=1
)
...
# %%
quality_df = (
    df.groupby(["model_name", "image_selection"], observed=False)["is_perfect_run"]
    .mean()
    .unstack()
)
...
outcome_counts = (
    df.groupby(["model_name", "outcome"], observed=False).size().unstack(fill_value=0)
)
...
outcome_pct = (
    outcome_counts.div(outcome_counts.sum(axis=1).replace(0, np.nan), axis=0) * 100
)
outcome_pct = outcome_pct.fillna(0)

# Enforce consistent column coloring and ordering
desired_order = [
    "Failed: Wrong Recipient",
    "Failed: Wrong Best Image",
    "Failed: Invalid JSON / Schema",
    "Failed: Other",
]
...
for c in ax_fail.containers:
    ...
    ax_fail.bar_label(
        c, labels=labels, label_type="center", fontsize=9, color="white", weight="bold"
    )
...
llama_ocr_df = (
    llama_ocr_df[llama_ocr_df["params"] > 0]
...
style_df = (
    df.groupby(["model_name", "doc_style"], observed=False)["is_perfect_run"]
    .mean()
    .unstack()
)
